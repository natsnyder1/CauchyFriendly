...
source $REPO_DIR/scripts/xsede_init.sh
#SBATCH parameters
ACCOUNT="cla299"
PARTITION=compute
TIME=02:30:00
...
CPUS_PER_TASK=128
MEM_REQ=128GB
CAUCHY_EXEC="leo_satellite_7state"
...
JOB_NAME="leo7-w4-TA1e28-TC-TR-1e9"
...
#Check whether the job name directory exists in ../log/
if [ -d ../log/$JOB_NAME ] && [ $CONTINUE_OR_START == "r"  ]; then
  echo "[ERROR:] JOB NAME $JOB_NAME subdirectory already exists at ../log/$JOB_NAME. Please manually delete this folder, or change the CONTINUE_OR_START=r configuration to CONTINUE_OR_START=c."
  exit 1
fi
...
sbatch --job-name=$JOB_NAME --partition=$PARTITION --nodes=$NODES --ntasks-per-node=$NTASKS_PER_NODE --cpus-per-task=$CPUS_PER_TASK --mem=$MEM_REQ --account=$ACCOUNT --export=ALL --no-requeue -t $TIME -e $JOB_CFG_DIR/$JOB_NAME.err -o $JOB_CFG_DIR/$JOB_NAME.out $REPO_DIR/scripts/run_job.sh $REPO_DIR/bin/$CAUCHY_EXEC $CAUCHY_EXEC_CMD_LINE_ARGS
if [ $? -eq 0 ]; then
