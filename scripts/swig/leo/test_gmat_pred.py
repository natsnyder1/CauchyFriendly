...
def store_data(cache_dic, fpath):
    input_ok = False
    while not input_ok:
        input_ok = True
...
def test_sat_pc_mc():
    ...
    cached_dir = "1715996963.2422733.pickle" # if set to something, loads and adds to this dir, if set to nothing, creates a new directory 
    ...
    if cached_dir is "":
        ...
        (p_xs, p_zs, p_ws, p_vs), (p_xs_kf, p_Ps_kf) = pc.simulate_then_run_ekf(t0, p_x0, P0_kf, filt_dt, sas_Cd, std_Cd, tau_Cd, sas_alpha, filt_orbits, std_gps_noise, with_density_jumps, STM_order, Wn, H, V)
        cache_dic['prim_sim'] = (p_xs.copy(), p_zs.copy(), p_ws.copy(), p_vs.copy())
    ...
    if cached_dir is "":
        ...
        (s_xs, s_zs, s_ws, s_vs), (s_xs_kf, s_Ps_kf) = pc.simulate_then_run_ekf(t0, s_x0, P0_kf, filt_dt, sas_Cd, std_Cd, tau_Cd, sas_alpha, filt_orbits, std_gps_noise, with_density_jumps, STM_order, Wn, H, V)
        cache_dic['sec_sim'] = (s_xs.copy(), s_zs.copy(), s_ws.copy(), s_vs.copy())
    ...
    if cached_dir is "":
        if with_no_initial_pred_error:
            ...
            p_pks, p_vks, p_cdks, p_aks, p_Pks_kf = pc.sat_prediction(t0_pred, p_xpred, p_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['kf_prim_pred'] = (p_pks.copy(), p_vks.copy(), p_cdks.copy(), p_aks.copy(), p_Pks_kf.copy())
            cache_dic['prim_pred'] = (p_pks.copy(), p_vks.copy(), p_cdks.copy(), p_aks.copy())
            ...
            s_pks, s_vks, s_cdks, s_aks, s_Pks_kf = pc.sat_prediction(t0_pred, s_xpred, s_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['kf_sec_pred'] = (s_pks.copy(), s_vks.copy(), s_cdks.copy(), s_aks.copy(), s_Pks_kf.copy())
            cache_dic['sec_pred'] = (s_pks.copy(), s_vks.copy(), s_cdks.copy(), s_aks.copy())
        else:
            ...
            p_Ppred = p_Ps_kf[-1].copy()
            p_pks_kf, p_vks_kf, p_cdks_kf, p_aks_kf, p_Pks_kf = pc.sat_prediction(t0_pred, p_xpred, p_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['kf_prim_pred'] = (p_pks_kf.copy(), p_vks_kf.copy(), p_cdks_kf.copy(), p_aks_kf.copy(), p_Pks_kf.copy())
            p_xpred = p_xs[-1].copy()
            p_Ppred = None
            p_pks, p_vks, p_cdks, p_aks = pc.sat_prediction(t0_pred, p_xpred, p_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['prim_pred'] = (p_pks.copy(), p_vks.copy(), p_cdks.copy(), p_aks.copy())
            ...
            s_xpred = s_xs[-1].copy()
            s_Ppred = None
            s_pks, s_vks, s_cdks, s_aks = pc.sat_prediction(t0_pred, s_xpred, s_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['sec_pred'] = (s_pks.copy(), s_vks.copy(), s_cdks.copy(), s_aks.copy())
        store_data(cache_dic, fpath)
    else:
    ...
    if run_itca:
        ...
        if with_no_initial_pred_error:
            cache_dic['kf_itca_data'] = (i_star_lhs, t_lhs, t_c, pp_c, pv_c, sp_c, sv_c)
        ...
        # If we have initial prediction error, run iterative time of closest approach over this window using the KFs predicted state trajectory and variances
            ...
            print("Running the Iterative Time of Closest Approach Algorithm for EKF Expected Encounter Plane")
            ...
            (s_xtc_kf, s_Ptc_kf) = pc.find_tca_and_prim_sec_stats(t0_pred, start_idx, end_idx, pred_dt, 
                ...
                sas_Cd, std_Cd, tau_Cd, sas_alpha, with_pred_plots)
            cache_dic['kf_itca_data'] = (i_star_lhs_kf, t_lhs_kf, t_c_kf, pp_c_kf, pv_c_kf, sp_c_kf, sv_c_kf)
    ...
    if run_mce:
        ...
        mce_num_windows = 3 # 5
        mce_dt = 300.0 # sec
        beta_dt = mce_dt # filt_dt # 60
        mce_init_func = gmce.reinitialize_func_H_summation #gmce.reinitialize_func_init_cond # reinitialize_func_H_summation # reinitialize_func_speyer
        ...
        mce_t0 = gmat_time_string_2_datetime(t0) + timedelta( seconds = mce_msmt_idx * filt_dt )
        
        mce_gamma = np.ones(3) * std_gps_noise * 1000  * ce.GAUSSIAN_TO_CAUCHY_NOISE
        ...
        beta_gauss = (std_Cd * ce.CAUCHY_TO_GAUSSIAN_NOISE) / (tau_Cd * (1.0 - np.exp(-beta_dt/tau_Cd)))
        ...
        if mce_start_by_time_prop:
            ...
            mce_x0[6] = 0.01
            fermSat = FermiSatelliteModel(mce_t0 - timedelta(seconds = mce_dt), mce_x0.copy(), mce_dt, gmat_print = False)
            ...
            mce_Phi0 = (np.eye(7) + mce_p_Jac * mce_dt + mce_p_Jac @ mce_p_Jac * mce_dt**2 / 2 + mce_p_Jac @ mce_p_Jac @ mce_p_Jac * mce_dt**3 / 6)
            mce_A0 = mce_Phi0.T
        ...
        # USE THE KF To Initialize the MCE
            ...
            mce_A0, mce_p0, mce_b0 = ce.speyers_window_init(mce_p_dx0, mce_p_P0, H[0], mce_gamma[0], mce_p_dz0)
        ...
        for pHat, apHat in zip(p_Ps_mce, p_Ps_mce_avg):
            ...
            apHat[6,0:6] /= 1000
            apHat[0:6,6] /= 1000
        cache_dic['mce_prim_sim'] = (mce_msmt_idxs, p_xs_mce, p_Ps_mce, p_xs_mce_avg, p_Ps_mce_avg)
        ...
        if mce_start_by_time_prop:
            ...
            mce_Phi0 = (np.eye(7) + mce_s_Jac * mce_dt + mce_s_Jac @ mce_s_Jac * mce_dt**2 / 2 + mce_s_Jac @ mce_s_Jac @ mce_s_Jac * mce_dt**3 / 6)
            mce_A0 = mce_Phi0.T
            mce_p0 = gmce.mce_naive_p0.copy()
            mce_b0 = np.zeros(7)
        ...
        # USE THE KF To Initialize the MCE
            ...
            mce_A0, mce_p0, mce_b0 = ce.speyers_window_init(mce_s_dx0, mce_s_P0, H[0], mce_gamma[0], mce_s_dz0)
        ...
        for pHat, apHat in zip(s_Ps_mce, s_Ps_mce_avg):
            ...
            apHat[6,0:6] /= 1000
            apHat[0:6,6] /= 1000
        cache_dic['mce_sec_sim'] = (mce_msmt_idxs, s_xs_mce, s_Ps_mce, s_xs_mce_avg, s_Ps_mce_avg)
        ...
        if with_no_initial_pred_error:
            ...
            cache_dic['mce_prim_pred'] = None #(p_pks_mce, p_vks_mce, p_cdks_mce, p_aks_mce, p_Pks_mce)
            ...
            cache_dic['mce_sec_pred'] = None #(s_pks_mce, s_vks_mce, s_cdks_mce, s_aks_mce, s_Pks_mce)           
            cache_dic['mce_itca_data'] = cache_dic['kf_itca_data'] # is same as expected: kf == mce == expected
            print("Stepping End of MCEs Filtration to Expected TCA For Primary Satellite")
            p_xtc_mce, p_Ptc_mce = pc.step_sat_stats_to_tca(t0_pred, p_xpred, p_Ppred, pred_dt, i_star_lhs, t_lhs, t_c, sas_Cd, std_Cd, tau_Cd, sas_alpha, STM_order)
            cache_dic['mce_prim_tca'] = (p_xtc_mce, p_Ptc_mce)
            print("Stepping End of MCEs Filtration to Expected TCA For Secondary Satellite")
            s_xtc_mce, s_Ptc_mce = pc.step_sat_stats_to_tca(t0_pred, s_xpred, s_Ppred, pred_dt, i_star_lhs, t_lhs, t_c, sas_Cd, std_Cd, tau_Cd, sas_alpha, STM_order)
            ...
            p_pks_mce, p_vks_mce, p_cdks_mce, p_aks_mce, p_Pks_mce = pc.sat_prediction(t0_pred, p_xpred, p_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['mce_prim_pred'] = (p_pks_mce, p_vks_mce, p_cdks_mce, p_aks_mce, p_Pks_mce)
            print("Running MCE Prediction For Secondary")
            s_pks_mce, s_vks_mce, s_cdks_mce, s_aks_mce, s_Pks_mce = pc.sat_prediction(t0_pred, s_xpred, s_Ppred, pred_dt, pred_steps, sas_Cd, std_Cd, tau_Cd, sas_alpha)
            cache_dic['mce_sec_pred'] = (s_pks_mce, s_vks_mce, s_cdks_mce, s_aks_mce, s_Pks_mce) 
            print("Running the Iterative Time of Closest Approach Algorithm for MCE Expected Encounter Plane")
            ...
            (s_xtc_mce, s_Ptc_mce) = pc.find_tca_and_prim_sec_stats(t0_pred, start_idx, end_idx, pred_dt, 
                ...
                sas_Cd, std_Cd, tau_Cd, sas_alpha, with_pred_plots)
            cache_dic['mce_itca_data'] = (i_star_lhs_mce, t_lhs_mce, t_c_mce, pp_c_mce, pv_c_mce, sp_c_mce, sv_c_mce)
            cache_dic['mce_prim_tca'] = (p_xtc_mce, p_Ptc_mce)
            cache_dic['mce_sec_tca'] = (s_xtc_mce, s_Ptc_mce)
        # Save Data 
        store_data(cache_dic, fpath)
    else:
        p_xtc_mce, p_Ptc_mce = cache_dic['mce_prim_tca']
        s_xtc_mce, s_Ptc_mce = cache_dic['mce_sec_tca']
        mce_msmt_idxs, p_xs_mce, p_Ps_mce, p_xs_mce_avg, p_Ps_mce_avg = cache_dic['mce_prim_sim']
        mce_msmt_idxs, s_xs_mce, s_Ps_mce, s_xs_mce_avg, s_Ps_mce_avg = cache_dic['mce_sec_sim']
        mce_steps = cache_dic['mce_info'][0][1]
        mce_dt = cache_dic['mce_info'][2][1]

    if True: #with_filter_plots:
        gmce.plot_against_kf(mce_msmt_idxs, p_xs, p_xs_kf, p_Ps_kf, p_xs_mce, p_Ps_mce, p_xs_mce_avg, p_Ps_mce_avg, filt_dt, mce_dt, sig = 1, title_prefix = 'Primary Satellite')
        gmce.plot_against_kf(mce_msmt_idxs, s_xs, s_xs_kf, s_Ps_kf, s_xs_mce, s_Ps_mce, s_xs_mce_avg, s_Ps_mce_avg, filt_dt, mce_dt, sig = 1, title_prefix = 'Secondary Satellite')
    ...
    # End of Filtration
    p_P_mce_filtend = cache_dic['mce_prim_sim'][2][-1] # [4][-1] # [4][-1] is average, [2][-1] is 'best'
    s_P_mce_filtend = cache_dic['mce_sec_sim'][2][-1] # [4][-1] # [4][-1] is average, [2][-1] is 'best'
