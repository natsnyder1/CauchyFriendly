...
def leo_5state_transition_model_jacobians(x):
    ...
    Gamma_k = np.zeros((x.size,3))
    Gamma_c = np.zeros((x.size,3)) # continous time Gamma 
    Gamma_c[2,0] = 1.0
    Gamma_c[3,1] = 1.0
    Gamma_c[4,2] = 1.0
    for i in range(taylor_order+1):
...
def test_leo3():
    ...
    if with_ce:
        ...
        x_bar0 = leo_5state_transition_model(leo.x0)[2:]
        A0 = np.eye(3) #leo_5state_transition_model_jacobians(xs[1])[0][2:,2:].T 
        p0 = np.array([noise_std, noise_std, leo.alpha_density_cauchy])
        b0 = np.zeros(3)
        beta = np.array([1e-8, 1e-8, leo.beta_cauchy])
        gamma = np.array([noise_std * leo.GAUSS_TO_CAUCHY, noise_std * leo.GAUSS_TO_CAUCHY])
        ce.set_tr_search_idxs_ordering([2,1,0])
        cauchy_steps = 4
        cauchyEst = ce.PyCauchyEstimator("nonlin", cauchy_steps, True)
        cauchyEst.initialize_nonlin(x_bar0, A0, p0, b0, beta, gamma, ece_dynamics_update_callback, ece_nonlinear_msmt_model, ece_extended_msmt_update_callback, 0)
    ...
    if with_cascade:
        x0 = xs[1][2:].copy()
        A0 = leo_5state_transition_model_jacobians(xs[1])[0][2:,2:].T #np.array([1,.1,.1,.1,1,.1,.1,.1,1]).reshape((3,3)) #leo_5state_transition_model_jacobians(xs[1])[0][2:,2:].T
        p0 = np.array([noise_std, noise_std, leo.alpha_density_cauchy])
        b0 = np.zeros(3)
        beta = np.array([leo.beta_cauchy])
        ce_noise_std1 = Ps_kf[-1][0,0]**0.5 * leo.GAUSS_TO_CAUCHY
        ce_noise_std2 = Ps_kf[-1][1,1]**0.5 * leo.GAUSS_TO_CAUCHY
        gamma = 3*np.array([ce_noise_std1, ce_noise_std2])
        ce.set_tr_search_idxs_ordering([2,1,0])
        #cauchy_steps = 8
        #cauchyEst = ce.PyCauchyEstimator("nonlin", cauchy_steps, True)
        ...
        win_var_boost = None
        log_dir = None
        cauchyEst = ce.PySlidingWindowManager("nonlin", 8, sim_steps, log_dir)
