...
C_COMPLEX_TYPE eval_term_for_2D_ucpdf_v2(double x1, double x2, double* A, double* p, double* b, GTABLE gtable, const int gtable_size, const int m)
{
    ...
    return MAKE_CMPLX( 2 * term_integral * RECIPRICAL_TWO_PI_SQUARED, 0); //(2 * term_integral * RECIPRICAL_TWO_PI_SQUARED) + 0*I;
}
...
double eval_2D_ucpdf_at_x(double x1, double x2, CauchyEstimator* cauchyEst)
{
    ...
    C_COMPLEX_TYPE term_ucpdf_val = MAKE_CMPLX(0,0); //0 + 0*I;
    C_COMPLEX_TYPE ucpdf_val = MAKE_CMPLX(0,0); //0 + 0*I;
...
struct PointWise2DCauchyCPDF
{
                ...
                double _gridy_resolution)
    {
        ...
        if(_dirname != NULL)
        {
            ...
            snprintf(grid_sizes_fname,len_dirname+34, "%s/%s", dirname, "cpdf_grid_elems.txt");
    ...
    void store_2d_cpdf(const int cpdf_idx)
    {
        ...
        assert(cpdf_idx >= 0);
        snprintf(cpdf_fname, len_dirname+24, "%s/%s%d%s", dirname, "cpdf_", cpdf_idx, ".bin");
        ...
        snprintf(str_idx_and_gridpoints, 20, "%d,%d,%d\n", cpdf_idx, num_gridx , num_gridy);
