...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    int size_out_Phat, size_out_cerr_fz, size_out_cerr_xhat;
    int size_out_cerr_Phat, size_out_err_code;

    pycauchy_single_step_ltiv(
        _pcdh,
        msmts, 1,   // left constant for seg fault checking
        controlValues, 0,   // left constant for seg fault checking
    ...
    // plhs[9] = mxCreateDoubleMatrix(size_out_cerr_fz, 1, mxREAL);
    // memcpy(mxGetPr(plhs[9]), out_cerr_fz, size_out_cerr_fz * sizeof(double));
    // free(out_cerr_fz);

    // plhs[10] = mxCreateDoubleMatrix(size_out_cerr_xhat, 1, mxREAL);
    // memcpy(mxGetPr(plhs[10]), out_cerr_xhat, size_out_cerr_xhat * sizeof(double));
    // free(out_cerr_xhat);

    // plhs[11] = mxCreateDoubleMatrix(size_out_cerr_Phat, 1, mxREAL);
    // memcpy(mxGetPr(plhs[11]), out_cerr_Phat, size_out_cerr_Phat * sizeof(double));
    // free(out_cerr_Phat);

    // plhs[12] = mxCreateNumericMatrix(size_out_err_code, 1, mxINT32_CLASS, mxREAL);
    // memcpy(mxGetData(plhs[12]), out_err_code, size_out_err_code * sizeof(int));
    // free(out_err_code);
}
