...
def generate_random_vectors(num_steps, W, V, x0_truth_mu, x0_truth_cov):
    np.random.seed(0)  # Seed for reproducibility
    ws = np.random.multivariate_normal(np.zeros(W.shape[0]), W, num_steps).T
    vs = np.random.multivariate_normal(np.zeros(V.shape[0]), V, num_steps).T
...
def simulate_gaussian_ltiv_system(num_steps, x0_truth, us, Phi, B, Gamma, W, H, V, with_zeroth_step_msmt = True, dynamics_update_callback = None, other_params = None):
    ...
    ws = ws if with_zeroth_step_msmt else ws[:, 1:]
    vs = vs[:, :num_steps] if with_zeroth_step_msmt else vs[:, 1:num_steps+1]
...
def run_python_simulation():
    ...
    generate_random_vectors(num_propagations, W, V, x0_kf, P0_kf)
