## Overview
This repository contains the C++/Python/Matlab interfaces for the Multivariate Cauchy Estimator (MCE) algorithm, compilable on Mac/Linux/Windows OS. The Python and Matlab wrappers (built via Swig, Mex, respectively) bind the Python/Matlab interfaces (modules) to the fast C++ backend library. See beneath the attached video for installation instructions. 

## What is the Multivariate Cauchy Estimator?
Much like the Kalman filter (KF) / Extended Kalman filter (EKF), the MCE / Extended MCE (EMCE) can be used for state estimation of linear time-invariant (LTI), linear time-varying (LTV) and non-linear dynamic systems. The MCE algorithm scales well to moderate state space dimensions of up to sizes seven or eight: similar to the limitations of a particle filter.
